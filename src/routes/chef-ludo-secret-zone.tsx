...
const recipeSchema = z.object({
  ...
  astuces: z.string().max(2000).optional(),
  featured: z.boolean(),
...
function parseDurationString(
  ...
  const hourMinuteMatch = normalized.match(/^(\d+)h(?:([0-5]?\d)(?:min)?)?$/);
  if (hourMinuteMatch) {
    return {
      hours: Number(hourMinuteMatch[1]),
      minutes: hourMinuteMatch[2] ? Number(hourMinuteMatch[2]) : 0,
    };
  }
...
function RecipeForm({
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const prep_time = Math.max(0, prepHours * 60 + prepMinutes);
      const cook_time = Math.max(0, cookHours * 60 + cookMinutes);
      ...
      // Supabase: on envoie prep_time et cook_time en minutes entières
      ...
      // Map chef_notes form state to the actual 'astuces' database column
      const astucesValue = form.chef_notes?.trim() ? form.chef_notes.trim() : null;
