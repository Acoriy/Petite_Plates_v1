...
import { RatingCard } from "@/components/site/RatingCard";
  ...
  head: () => ({
    meta: [
      ...
      // Google AdSense Account Meta Tag
      {
        ...
        content: "ca-pub-1799331850346217",
      },
    ],
    links: [{ rel: "canonical", href: "/" }],
    // Inject the Google AdSense Script securely into the document head
    scripts: [
      {
        src: "https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-1799331850346217",
        async: true,
  ...
  return (
    ...
    >
      ...
      <div className="mt-5 space-y-2.5">
        ...
        <a
          href={`https://www.paypal.com/donate?business=${encodeURIComponent("ludovicdeliere53@gmail.com")}&currency_code=EUR`}
          target="_blank"
          rel="noopener noreferrer"
          className="group flex items-center justify-between rounded-full border-2 border-[var(--espresso)] bg-[var(--cream)] px-5 py-3 text-sm font-semibold text-[var(--espresso)] shadow-[3px_3px_0_0_var(--espresso)] transition-all hover:-translate-y-0.5 hover:shadow-[5px_5px_0_0_var(--espresso)]"
        >
          <span className="inline-flex items-center gap-2.5">
            <svg
              role="img"
            ...
            >
              <path d="M20.071 6.131c.325-1.996-.201-3.376-1.428-4.382C17.309.682 15.118.25 12.607.25H4.152a1.002 1.002 0 0 0-.987.834L.278 19.688a.501.501 0 0 0 .493.578h4.512l1.323-8.15c.026-.164.168-.282.335-.282h2.242c3.553 0 6.36-1.448 7.182-5.632.316-1.611.144-3.053-.746-4.071zm-4.707 5.174c-.531 2.71-2.614 3.651-5.322 3.651H7.838l1.01-6.223c.013-.082.084-.141.168-.141h1.16c1.696 0 3.14.28 3.905.955.558.491.733 1.258.483 2.758z" />
            </svg>
            Faire un don
          </span>
          <span className="font-serif italic text-base text-[var(--caramel)]">
            Offrir un café →
          </span>
        </a>
      </div>
                      ...
                      <div className="absolute inset-x-3 bottom-3 flex flex-wrap items-center justify-between gap-x-3 gap-y-1 rounded-xl border-2 border-[var(--espresso)] bg-[var(--cream)] px-3 py-2 text-[11px] font-semibold text-[var(--espresso)] shadow-[3px_3px_0_0_var(--espresso)]">
                        <span className="inline-flex items-center gap-1.5">
                          ...
                          {r.prep_time != null || r.cook_time != null
                            ? `${formatDuration(r.prep_time)}${r.cook_time != null ? " + " + formatDuration(r.cook_time) : ""}`
                            : "—"}
                        </span>
                        <span className="inline-flex items-center gap-1.5">
