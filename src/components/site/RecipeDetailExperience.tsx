...
export function RecipeDetailExperience({
  ...
  const totalTime = `${formatDuration(recipe.prep_time)}${recipe.cook_time != null ? ' + ' + formatDuration(recipe.cook_time) : ''}`;
  ...
  return (
    <article className="relative pb-28 md:pb-24">
      ...
      <section className="container relative z-20 mx-auto -mt-12 px-4 md:px-8">
        ...
        >
          <StatPill icon={<Clock className="h-5 w-5" />} label="Préparation" value={formatDuration(recipe.prep_time)} />
          <StatPill icon={<Flame className="h-5 w-5" />} label="Cuisson" value={formatDuration(recipe.cook_time)} />
          ...
          <section id="recipe-intro" className="scroll-mt-36">
            ...
            >
              ...
              <ul className="mt-8 flex flex-wrap gap-3 text-sm font-semibold text-[var(--espresso)]">
                <li className="rounded-full bg-[var(--cream)] px-4 py-2 shadow-sm">⏱️ {totalTime} au total</li>
    ...
    <div>
      <div className="text-center">
        ...
        <p className="mt-3 text-sm text-[var(--espresso)]/65">
          {totalTime ?? "Temps de préparation indisponible"} · {instructions.length} étapes · Cliquez pour marquer comme fait
        </p>
      </div>
